...
try:
    from experiments.experiment_2.src.emotion_labeler import EmotionLabeler
except ImportError:
    try:
        from src.emotion_labeler import EmotionLabeler
    except ImportError:
...
def prepare_eda_dataset(input_file: str = None, sample_size: int = 50000) -> pd.DataFrame:
    ...
    if input_file is None:
        input_file = str(WORKSPACE_ROOT / "data" / "processed" / "twcs_cleaned.csv")
...
if __name__ == '__main__':
    default_input = str(WORKSPACE_ROOT / "data" / "processed" / "twcs_cleaned.csv")
